...
def send_telegram_photo(
    ...
    """Send 1 single cropped sample photo to Telegram chat(s) via Bot API sendPhoto."""
    tokens = [bot_token] if bot_token else parse_list_from_env("TELEGRAM_BOT_TOKEN")
    targets = chat_ids or parse_list_from_env("TELEGRAM_CHAT_ID")
    ...
    for token in tokens:
        ...
        for chat_id in targets:
            files = {
                "photo": ("code_sample_crop.png", photo_bytes, "image/png")
            }
            data = {
                "chat_id": chat_id,
            }
            ...
            try:
                logger.info(f"Sending 1 single cropped sample photo to Telegram chat_id {chat_id}...")
                response = requests.post(api_url, data=data, files=files, timeout=20)
                if response.status_code == 200:
                    logger.info(f"Telegram cropped sample photo sent successfully to {chat_id}!")
                else:
...
def notify_new_reward_codes(
    ...
    """Send all raw code text messages 1-by-1 first, then send EXACTLY 1 single cropped sample photo at the end."""
    codes = (small_codes or []) + (large_codes or [])
    if not codes:
        return True

    success = True
    # Step 1: Send each raw code string in its own clean text message
    for code in codes:
    ...
    # Step 2: AFTER all text messages are sent, send EXACTLY 1 single cropped sample photo at the end
    if sample_cropped_bytes:
        photo_res = send_telegram_photo(sample_cropped_bytes, caption=f"📸 Verification Sample Crop ({time_label})")
        if not photo_res:
            logger.warning("Failed to send 1 single verification sample photo to Telegram.")
